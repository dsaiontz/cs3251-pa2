...
class judge:
    ...
    def test_illegal_input(self, type, name):
        ...
        self.file.flush()
        cmds=[' ', ' 127.0.0.1',' 324.1.1.4 13001 cx1', ' 127.0.0.1 -3 cx1', ' 127.0.0.1 80 cx1', ' 127.0.0.1 13001 ""']
        for postfix in cmds:
            cmd = self.run[type] + name + postfix
            self.file.write('\nrun command: '+ cmd +'\n')
            self.file.flush()
    ...
    def runTest(self, type='p3', port=13001, srv = True):
...
if __name__ == '__main__':
    import sys
    args = sys.argv
    judge().runTest(args[1],  13001, True)